...
#[derive(Debug, Clone)]
pub(super) struct WindowedFilter<T, I, D> {
    window_length: D,
...
{
    pub(super) fn new(window_length: D) -> Self {
        WindowedFilter {
            window_length,
    ...
    pub(super) fn reset(&mut self, new_sample: T, new_time: I) {
    ...
    pub(super) fn get_best(&self) -> Option<T> {
        self.estimates[0].as_ref().map(|e| e.sample)
    }

    pub(super) fn get_second_best(&self) -> Option<T> {
        self.estimates[1].as_ref().map(|e| e.sample)
    }

    pub(super) fn get_third_best(&self) -> Option<T> {
    ...
    pub(super) fn update(&mut self, new_sample: T, new_time: I) {
